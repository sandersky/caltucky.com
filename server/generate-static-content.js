...
const {generatePageContent, generatePostContent} = require('./utils')
const blog = require('./wordpress').default

const needsRendered = []

function fetchPages(options) {
  return blog.pages({...options, order: 'asc', orderby: 'date'}).then(pages => {
    pages.forEach(page => {
...
function fetchPosts(options) {
  return blog.posts({...options, order: 'asc', orderby: 'date'}).then(posts => {
    posts.forEach(post => {
